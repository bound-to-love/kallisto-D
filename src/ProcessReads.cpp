  ...
  for (int i = 0; i < seqs.size(); i++) {
    ...
    */
    ...
    if (!mp.opt.single_overhang && !u.isEmpty() && (!paired || v1.empty() || v2.empty()) && (tc.has_mean_fl || (mp.opt.long_read && !v1.empty()))) {
	  ...
	  if (!mp.opt.long_read || (mp.opt.long_read && x.first < 200)){
            vtmp.add(tr);
	  }
        } else {
          continue;//pass
        ...
        if (!x.second && x.first - fl >= 0) {
          if (!mp.opt.long_read || (mp.opt.long_read && ((int)index.target_lens_[tr] - x.first) < 200)){
            vtmp.add(tr);
	  }
        } else {
          continue;//pass
    ...
    // find the ec
    if (!u.isEmpty()) { //&& (!mp.opt.long_read || (mp.opt.long_read && u.cardinality() == 1))) { //THIS IS FOR ONLY UNIQUELY ALIGNING LONG READS 
...
//    auto &bcc = busopt.bc[0];
    ...
    if (busopt.long_read && !u.isEmpty()) {
	  ...
	  if (!busopt.long_read || (busopt.long_read && x.first < 200)){
            vtmp.add(tr);
	  }
        } else {
          continue;//pass
        ...
        if (!x.second && x.first - (seqlen - 30) >= 0) {
          if (!busopt.long_read || (busopt.long_read && ((int)index.target_lens_[tr] - x.first) < 200)){
            vtmp.add(tr);
	  }
        } else {
          continue;//pass
