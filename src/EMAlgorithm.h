    ...
    if (opt.long_read){
     ...
     for (int i = 0; i < tc.flens_lr.size(); i++){
       if  (tc.flens_lr_c[i] < 0.000001){
         eff_len = 50; //(int)index_.target_lens_[i]; //if we are not seeing this transcript uniquely mapping at all then just use the length of the transcript 
       } else {
         eff_len = 50; //(double)index_.target_lens_[i] - std::fabs(double(tc.flens_lr[i])/double(tc.flens_lr_c[i]));
         ...
         if (eff_len < 1.0) {
           ...
           eff_len = 50; //1; //if the average length of a read mapping within a transcript is within 1 of the target length then the average number of positions that the read could begin at is the first.  
         } 
       }
